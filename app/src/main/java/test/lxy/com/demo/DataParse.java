...
*/
     ...
     * 找人要均线计算公式
     */
    public void parseMinutesData(ArrayList<FsBean2> list) {
        ...
        for (int i = 0; i < count; i++) {
            MinutesBean minutesData = new MinutesBean();
            FsBean2 bean = list.get(i);

            String[] strings = bean.time.split(" ");//2016-10-21 09:32:00
            String s = strings[1].substring(0, 5);


            minutesData.time = s;
            minutesData.cjprice = bean.close;//成交价

            if (i != 0) {
                minutesData.cjnum = bean.volume;
                minutesData.total = bean.balance;
                minutesData.avprice = (minutesData.total) / bean.volume/100;//均价

            } else {
                minutesData.cjnum = bean.volume;//成交量
                minutesData.avprice = bean.open;//均价
                minutesData.total = bean.balance;//交易额

            }
            minutesData.cha = minutesData.cjprice - baseValue;//6.04 差值
            minutesData.per = (minutesData.cha / baseValue);//10.04  差值百分比
            double cha = bean.close - baseValue;//6.03999  差值
        ...
        System.out.println("perperper======="+permaxmin);
    }
